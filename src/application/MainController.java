...
//bouton de validation d'action
	...
	public void nouvellePartie(ActionEvent e) {
		...
		disableAll(); 										//on d√©sactive tous les boutons 
		...
		for (Tuiles tuile : this.listeTuiles){				//parcours les tuiles du plateau
			switch (tuile.getPosition()) {
				/*
					les diff√©rents case servent √† identifier le bouton auquel il faut changer le style
					on est oblig√© de remove tous les style possible
					puis d'ajouter le style souhait√©
				*/
				case 0:
		...
		while (this.tours<9 && this.joueur1.getTemps()<9) {									// 8 tours max en commencant par le 1er
			int[] hazard = {RandInt(0,1),RandInt(0,1),RandInt(0,1),RandInt(0,1)};			//on gÈnËre les jetons alÈatoirements
			...
			//Les enquÍteurs/Mrjack jouent les deux suivants
