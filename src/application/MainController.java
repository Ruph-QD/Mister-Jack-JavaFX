...
//on r√©cup√®re les boutons actions
...
//pour stocker les inspecteurs
	...
	public void nouvellePartie(ActionEvent e) {
		...
		this.joueur1 = new Joueur( ((this.fieldJack.getText()=="" ) ? "MrJack" : this.fieldJack.getText()), "MrJack");				//si le champs est vide, on donne le nom par d√©fault
		this.joueur2 = new Joueur( ((this.fieldInspe.getText()=="" ) ? "Inspecteur" : this.fieldInspe.getText()), "Inspecteur"); 	//si le champs est vide, on donne le nom par d√©fault

		//Creation de la pioche
		this.pioche= new Pioche();
		this.pioche.Piocher(this.joueur1); 					//pour donner la carte alibi sous lequel MrJack est cach√©e
		this.MrJack.setDisable(false);
		//Creation du jeu
		this.plateau = new Plateau(); 						//cr√©er le plateau
	...
	//partie creation des inspecteurs et des boutons associ√©s
	...
	public void nouveauTour(){
		int[] hazard = {RandInt(0,1),RandInt(0,1),RandInt(0,1),RandInt(0,1)};			//on gÔøΩnÔøΩre les jetons alÔøΩatoirements
	...
	public void finduJeu1() { 																					//Voil‡ la fonction qui cÈlËbre le vainqueur en fonction du rÈsulat de l'ultime test
		System.out.println("GAME OVER");
		this.joueurActu.setText("BRAVO a† Mr Jack qui GAGNE cette partie");
	}
	
	public void finduJeu2() { 																					//Voil‡ la fonction qui cÈlËbre le vainqueur en fonction du rÈsulat de l'ultime test
		System.out.println("GAME OVER");
	...
	public void utiliserJetons(){
		if (this.joueur1.getTemps()>=8) {finduJeu1();}															//on vÈrifie que Mr.Jack n'est pas gagner aprËs chaque jetons
		switch(this.jetonsUtilise) {
			...
			case 0:
				...
				this.joueurActu.setText("C'est √† " + this.joueurActuel.getNom() + " de jouer");
				this.jetonsUtilise++;																			//chaque jeton est comptabilisÈ pour savoir comment se dÈroule de tour
				break;
			case 1:
				this.jetonsUtilise++;
				break;
	...
	public void deplacerTobi() {  		// Methode pour deplacer inspecteur Tobi de une ou deux cases apres avoir appuye sur son jeton
		...
		if (reset == 0) {action21.setDisable(true);}else {this.reset=0;} // si l'appel rÈsulte du jetons choix de l'inspecteur, alors le jetons 21 ne doit pas Ítre dÈsactiver
	}
		...
		if (this.joueurActuel==joueur2){					//on retroune la carte si c'est l'inspecteur
			for (int k =0;k<this.listeTuiles.size();k++){
				if (this.listeTuiles.get(k).getImage(0).equals('T'+carte)){
					...
					this.listeTuiles.get(k).setImageAffichee(listeTuiles.get(k).getImage(this.listeTuiles.get(k).getAngle()));	//on change l'image affichÈe par la console
					((Button) borderPane.lookup("#button"+k)).getStyleClass().removeAll("TBert","TBert90","TBert180","TBert270","TGoodley","TGoodley90","TGoodley180","TGoodley270","TGull","TGull90","TGull180","TGull270","TLane","TLane90","TLane180","TLane270","TLestrade","TLestrade90","TLestrade180","TLestrade270","TMadame","TMadame90","TMadame180","TMadame270","TPizer","TPizer90","TPizer180","TPizer270","TSmith","TSmith90","TSmith180","TSmith270","TStealthy","TStealthy90","TStealthy180","TStealthy270","Verso","Verso90","Verso180","Verso270","Middle_verso");
					((Button) borderPane.lookup("#button"+k)).getStyleClass().addAll(this.listeTuiles.get(k).getImageAffichee()); //on change l'image affichÈe par javaFX
				}
			}
		}
		this.tempsAffiche.setText("x "+joueur1.getTemps()); //on met √† jour le temps affich√©
	...
	public void pushed(ActionEvent e){
		switch(jetonSelect) {
			...
			case 11://intervertir Tuiles : On doit appuyer deux fois pour effectuer le changement. La prmei√®re fois, la tuile est sauvegard√©e dans tuileSelectionne puis on fait l'inversion des deux styles
				if (tuileSelectionne.size()>0){
					...
					plateau.IntervertirTuiles(this.listeTuiles.get(numero0), this.listeTuiles.get(numero1));							//on fait les modifications Ègalement sur les classe java (en console)
			...
			case 12://tourner Tuiles : la choix de la tuile ‡ tourner se verouille au premier click mais la tuile peut encore Ítre tounÈe ‡ chque click jusqu'‡ validation
				enableTuiles(false);
			...
			case 41://choisir inspecteurs : On clique sur l'inspecteur que l'on souhaite dÈplacer
	...
	public void appelTemoin(){ //R√©alise l'appel √† t√©moin
		...
		//un parcours de boucle sert √† faire la vision d'un inspecteur
		
		for (int k=0; k<3;k++){
			switch (Integer.parseInt(positionInsp[k])) {
				/*Suivant la position on a 12 cas possibles
				On regarde √† chaque fois s'il y a un mur devant l'inspecteur. Si ce n'est pas le cas, on stoke la tuile dans tuilesVisibles
				On regarde si le mur se trouve √† l'oppos√© de l'inspecteur (donc angle+2) et si le mur de la prochaine tuile ne se toruve pas devant l'inspecteur. On stocke si ce n'est pas le cas
				On refait la m√™me op√©ration que la derni√®re fois*/
				case 1:
		...
		if (!mrJackVisible){						//si MrJack n'ets pas visible
			for (int k =0; k<tuilesVisibles.size();k++){
				if(tuilesVisibles.get(k).getAngle()<4){	
													//si la tuile n'est pas d√©j√† retourn√©e
					...
					tuilesVisibles.get(k).setImageAffichee(tuilesVisibles.get(k).getImage(tuilesVisibles.get(k).getAngle()));	//on change l'image affich√©e
			...
			for (int k =0;k<listeTuiles.size();k++){
				if(!tuilesVisibles.contains(listeTuiles.get(k))){
					...
					listeTuiles.get(k).setImageAffichee(listeTuiles.get(k).getImage(listeTuiles.get(k).getAngle()));	//on change l'image affich√©e
		...
		this.tempsAffiche.setText("x " + this.joueur1.getTemps());	//on met √† jour l'affichage du temps
	}
	...
	private static int RandInt(int min, int max) { 	//gÈnËre un nombre alÈatoire entier entre min et max
